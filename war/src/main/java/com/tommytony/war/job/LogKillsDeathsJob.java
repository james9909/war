     ...
     */ public void run() {
        ...
        try {
            ...
            createStmt.executeUpdate(
                "CREATE TABLE IF NOT EXISTS `war_kills` (`date` datetime NOT NULL, `player` varchar(16) NOT NULL, `kills` int(11) NOT NULL, `deaths` int(11) NOT NULL, KEY `date` (`date`)) "
            ...
            conn.commit();
            stmt.close();
            final String deleteClause = War.war.getMysqlConfig().getLoggingDeleteClause();
            if (!deleteClause.isEmpty()) {
                Statement deleteStmt = conn.createStatement();
                deleteStmt.executeUpdate("DELETE FROM war_kills " + deleteClause);
                deleteStmt.close();
                conn.commit();
            }
        } catch (SQLException ex) {
            War.war.getLogger().log(Level.SEVERE, "Inserting kill-death logs into database", ex);
        } finally {
            if (conn != null) {
                try {
                    conn.close();

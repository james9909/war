...
public class WarPlayerListener implements Listener {
            ...
            if (team != null) {
                ...
                if (zone.isFlagThief(player)) {
                    ...
                    if (item != null) {
                        ...
                        if (zone.getLoadoutSelections().keySet().contains(player.getName()) && zone.getLoadoutSelections().get(player.getName()).isStillInSpawn()) {
                            // still at spawn
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        if (War.war.isLoaded()) {
            ...
            Warzone zone = Warzone.getZoneByPlayerName(player.getName());
            if (zone != null && zone.getLoadoutSelections().containsKey(player.getName()) && zone.getLoadoutSelections().get(player.getName()).isStillInSpawn()) {
            ...
            if (zone != null && team != null && event.getAction() == Action.RIGHT_CLICK_BLOCK && event.getClickedBlock().getState() instanceof InventoryHolder && zone.isFlagThief(player)) {
                event.setCancelled(true);
                War.war.badMsg(player, "drop.flag.disabled");
            }
    ...
    public void onPlayerMove(final PlayerMoveEvent event) {
        ...
        if (hub != null && hub.getVolume().contains(player.getLocation())) {
            ...
            if (zone != null && zone.getTeleport() != null) {
                if (zone.getWarzoneConfig().getBoolean(WarzoneConfig.AUTOJOIN) && zone.getTeams().size() >= 1 && currentTeam == null) {
        ...
        if (playerWarzone != null) {
            ...
            } else if (loadoutSelectionState != null && !loadoutSelectionState.isStillInSpawn() && !playerWarzone.isCakeThief(player) && (flagReturn.equals(FlagReturn.BOTH) || flagReturn
                .equals(FlagReturn.SPAWN)) && !playerWarzone.isFlagThief(player)) {
...
//				}
            ...
            if (!playerTeam.isSpawnLocation(player.getLocation()) && playerWarzone.getLoadoutSelections().keySet().contains(player.getName()) && playerWarzone.getLoadoutSelections()
                .get(player.getName()).isStillInSpawn()) {

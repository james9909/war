...
//					updateFromVersionTwoToTree(zoneName, databaseConnection);
    ...
    public static int load(Connection databaseConnection, ZoneVolume volume, World world, boolean onlyLoadCorners, int start, int total, boolean[][][] changes) throws SQLException {
        ...
        while (query.next()) {
            ...
            try {
                ...
                War.war.getLogger()
                    .log(Level.WARNING, "Exception loading some tile data. x:" + x + " y:" + y + " z:" + z + " type:" + modify.getType().toString() + " data:" + modify.getData().toString(), ex);
            }
        }
        query.close();
        stmt.close();
    ...
    /**
     * Finds the correct location to place a painting based on its characteristics. Credit goes to whatever forum I got this from.
     *
    ...
    public static int saveStructure(Volume volume, Connection databaseConnection) throws SQLException {
        ...
        stmt.executeUpdate("CREATE TABLE IF NOT EXISTS " + prefix + "_blocks (x BIGINT, y BIGINT, z BIGINT, type TEXT, data SMALLINT)");
        stmt.executeUpdate("CREATE TABLE IF NOT EXISTS " + prefix + "_corners (pos INTEGER PRIMARY KEY NOT NULL UNIQUE, x INTEGER NOT NULL, y INTEGER NOT NULL, z INTEGER NOT NULL)");
        stmt.executeUpdate("DELETE FROM " + prefix + "_blocks");
        stmt.executeUpdate("DELETE FROM " + prefix + "_corners");
        stmt.close();
        PreparedStatement cornerStmt = databaseConnection.prepareStatement("INSERT INTO " + prefix + "_corners SELECT 1 AS pos, ? AS x, ? AS y, ? AS z UNION SELECT 2, ?, ?, ?");
        ...
        cornerStmt.close();
        PreparedStatement dataStmt = databaseConnection.prepareStatement("INSERT INTO " + prefix + "_blocks VALUES (?, ?, ?, ?, ?)");
        databaseConnection.setAutoCommit(false);
        final int batchSize = 1000;
        int changed = 0;
        for (BlockState block : volume.getBlocks()) {
            final Location relLoc = ZoneVolumeMapper.rebase(volume.getCornerOne(), block.getLocation());
            ...
            for (int j = 0, y = volume.getMinY(); j < volume.getSizeY(); j++, y++) {
                for (int k = 0, z = volume.getMinZ(); k < volume.getSizeZ(); k++, z++) {
                    ...
                    if (state instanceof Sign) {
                        ...
                        dataStmt.setString(6, String.format("%s\n%s\n%s", ((Skull) block.getState()).hasOwner() ? ((Skull) block.getState()).getOwningPlayer().getName() : "",
                            ((Skull) block.getState()).getSkullType().toString(), ((Skull) block.getState()).getRotation().toString()));
    ...
    static Location rebase(final Location base, final Location exact) {
        return new Location(base.getWorld(), exact.getBlockX() - base.getBlockX(), exact.getBlockY() - base.getBlockY(), exact.getBlockZ() - base.getBlockZ());
    }
    ...
    private static void updateFromVersionOneToTwo(String zoneName, Connection connection) throws SQLException {
        ...
        stmt.executeUpdate(
            "CREATE TEMPORARY TABLE blocks_backup(x BIGINT, y BIGINT, z BIGINT, type TEXT, data SMALLINT, sign TEXT, container BLOB, note INT, record TEXT, skull TEXT, command TEXT, mobid TEXT)");
